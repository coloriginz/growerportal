/**
 * Submit loose PDF files from private_input/salessheets voorbeelden/ to the import API.
 * Wraps each PDF as a fake email payload.
 *
 * Usage: node scripts/submit-pdfs.js
 */
...
const DIR = path.join(__dirname, "..", "private_input", "salessheets voorbeelden");
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (res.status === 201) {
        ...
        console.log(`OK    ${file} -> ${p} processed, ${skippedPdfs.length} skipped`);
        if (skippedPdfs.length > 0) {
          skippedPdfs.forEach((s) => console.log(`      ${s.fileName}: ${s.reason}`));
        }
      } else {
