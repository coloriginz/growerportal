...
- When the sales sheet does not print a net label — roughly two-thirds of the time — it is derived as turnover minus costs, with missing costs counted as zero: a delivery with no cost line has no costs.
- **Threshold: EUR 1**, as the exported constant `SALESSHEET_MATCH_TOLERANCE`. Measured over 3.915 read sales sheets on test: 94,7% match and 5,3% differ by more than a euro; the difference below that is rounding, since `SalesSheetCost.amount` carries five decimals and the sales sheet sums before it rounds.
