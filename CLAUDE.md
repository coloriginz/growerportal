...
- **Fabric owns the supplier a delivery belongs to, and the portal follows it.** `Lot.supplierId`, `Lot.salesSheetId` and `SalesSheet.supplierId` are written on *update* as well as on insert, so a delivery reattributed in `marts.fct_partijen` moves in the portal at the next lots round. Fixing only the `ON CONFLICT` clause does nothing — existing rows take the separate update path, which is why two deliveries sat under the wrong supplier for months. When the new relation is not a portal supplier the partij is skipped and the delivery stays put; that case is reported apart from the unknown relations, as `ImportBatch.details.reattributedAway` (`{relId: {leveringen, van: [codes]}}`), because it means a supplier is looking at someone else's turnover. Activating the relation is the whole fix; the move follows by itself.
