...
- **LotCorrection** — Volume/colli correction on a lot. Has facttypeSub ("correctie"/"productiecorrectie") and `correctionReasonId`, which points at `CorrectionReasonCode.id` **without a foreign key** — that key would break a clean rebuild on a code that has not loaded yet, so the lookup is a separate query. `correctionDate` is the booking date from `marts.dim_partijcorrecties`. A counter-booking on an order line carries the original sale's date instead, but it is not undatable: every such correction is booked on its own **credit invoice**, and `Transaction.creditInvoiceDate` now holds that date — see the note on that field.
- **CorrectionReasonCode** — Lookup table for correction reasons from Fabric. Has code, Dutch/English names, type.
- **Transaction** — Individual sale (orderregel). `afterSettlement` marks a sale booked after the date printed on the settlement (`SalesSheet.pdfInvoiceDate`); those do not count anywhere in the portal — not in `Lot.totalAmount`, not in `SalesSheet.totalTurnover`, not on the sales, dashboard or shipment screens. The settlement is a snapshot and what lands on a delivery afterwards is not on the paper the grower holds, so counting it made the portal disagree with its own settlement. Measured on test: 38 sales over 27 deliveries, 30.915 stems, EUR 2.178,68 — hiding them moved 19 of those 27 to an exact match and broke none, taking the turnover reconciliation from 3.678 to 3.697 deliveries within EUR 1. **A correction is never hidden**, whatever its date: it reverses an earlier sale, so removing it would raise turnover instead of lowering it. That is a hard condition in `markAfterSettlement()`, not a reliance on counter-bookings carrying the original sale's date. The flag is set by the orders import and recomputed when a PDF is linked or unlinked — only then is `pdfInvoiceDate` known, or gone again. `src/lib/sync/after-settlement.ts`; `scripts/backfill-after-settlement.ts` is the catch-up round. **Turnover is deliberately hidden here**, so keep it measurable: `SELECT COUNT(*), SUM(amount) FROM "Transaction" WHERE "afterSettlement"`. Maps to `ordreg_id` in Fabric. Has salesType (VMP/Aurora/Veilen/Persoonlijk), stems, pricePerStem, amount. `creditInvoiceNumber`/`creditInvoiceDate` are filled on correction rows only and come from `intermediate.int_order_correctie` joined to `staging.stg_kbtpro__fact` — one layer below the marts, because `marts.fct_orders` does not carry them. This is the only attribute that predicts whether a correction appears on the grower's settlement. The reason code does not: all nine VRK codes occur in near-identical proportions on deliveries that match and on deliveries that differ. The credit invoice date does — measured over 438 deliveries, 90% fall on the right side of `SalesSheet.pdfInvoiceDate` (credit after printing → missing from the paper, 66 against 21; credit before → on it, 328 against 23). Exposure on test: 103 deliveries, EUR 13.746 that the portal subtracts and the document does not show. Reading the intermediate layer is a deliberate risk — it is dbt internals and changes shape sooner than the marts, which already do so unannounced; the durable place is the mart.
...
- `SalesSheet.invoiceNumber` is unique. **It is not an invoice number**: it holds the flight/container number the grower sends with the shipment (`inkoopfactuurnummer` in Fabric), which is why the screens call it "Shipment Number". The invoice number is `ourInvoiceNumber`, read off the sales sheet PDF. Because the column is unique and these numbers recycle per year, a collision gets `-<parthdrId>` appended, and a source value that says nothing (`xxx`, `volgt`, empty) becomes `FABRIC-<parthdrId>`
...
- **A 502 has two different causes, and telling them apart saves hours.** A column that does not exist comes back as a 502 within a second and looks like an outage — that is a typo in the query. A 502 on `SELECT TOP 0 *`, which names no column and reads no row, means the table object itself cannot be resolved. The six `ssh_*` settlement tables in `lh_landing.kbtpro` are of the second kind: catalogued in `_info_schema_columns` (a table the ingestion writes about the *source*) but never landed, so the settlement side is absent from the data landscape entirely. Neighbouring tables in the same schema read fine — `ordreg` holds 3.242.347 rows. See [afrekenprijs-en-afrekentabellen.md](docs/afrekenprijs-en-afrekentabellen.md), which also records that `marts.fct_orders.afrekenprijs_per_steel` is not always the settlement price: where `lh_landing.kbtpro.ordreg.afrekenprijsok` is false the mart still shows a figure, 153 consignment lines and EUR 14.938,85 over July 2026 alone.
...
- **`resolveShipmentStatus()` itself skips corrections, deliberately** — see the reasoning in `shipment-status.ts`. But delivered and sold only agree once corrections are added back in: `LotCorrection.correctionVolume` is meaningfully signed, and delivered + corrections lands on sold, not delivered alone. Delivery 2700240 (COLXLNFW) measures it exactly: 55,870 delivered, −33,380 in corrections, 22,490 sold — matching the printed sales sheet to the stem. `/api/admin/shipment-issues` (`stem-gap`) does add corrections back in, because there the comparison has to catch a real gap rather than approximate a phase transition; see `STEM_GAP_MARGIN` in that route for the margin and its measurement. **Both correction tracks count there, not just the lot one.** A correction on an order line lowers sold — Fabric reverses the sale and books it again — but it belongs to the delivered volume just as much as a lot correction does. Lot 3588009 (PCFFARCO) shows it: 14.400 delivered, −50 "less in box" as a lot correction, −180 "less delivered than ordered by customer" as an order-line correction, 14.170 sold. The settlement prints all 14.400 because it lists those written-off stems as zero-value lines. Leaving the order-line term out reported a gap of 180 on a delivery that reconciles to the cent: measured on test, 780 settled deliveries were flagged and 12 remain, with none newly flagged.
...
- **Fabric sends dates without a timezone, and `new Date()` reads that as local time.** `"2025-07-14T00:00:00"` becomes 22:00 the previous day in Europe/Amsterdam, so every import run outside UTC shifts deliveries back a day. Vercel runs UTC so production never shows it — which is exactly what makes it a trap for repair scripts run from a laptop. All four import routes go through `parseFabricDate()` (`src/lib/sync/fabric-date.ts`, covered by `scripts/checks/fabric-date.ts`); use it for any new Fabric date.
