    ...
    async (row: SkippedRelationRow) => {
      ...
      try {
        const res = await fetch("/api/admin/fabric-relations", {
          ...
          body: JSON.stringify({ fabricId: row.relId, companyId: chosenCompanyId }),
        });
        ...
        setActivated((prev) => new Set(prev).add(row.relId));
        toast.success(`Supplier ${body?.code ?? row.code ?? row.relId} created`, {
          description:
